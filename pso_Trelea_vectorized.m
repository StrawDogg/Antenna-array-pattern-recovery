...
% 控制台输出
message = sprintf('%%g/%g iter, GBest = %%e, step = %%e.\n',me);
...
for i=1:me  % start epoch loop (iterations)
    ...
    if plotflg==1
        if (rem(i,df) == 0 ) | (i==me) | (i==1)
            if i==1
                ...
                lastbestpos = bestpos(i-1,1:D);
            end
            fprintf(message,i,gbestval,norm(bestpos(i,1:D)-lastbestpos));
    ...
    elseif trelea ==3
        ...
        ac22 = rannum2.*ac2;
        if cnt2>10
            vel = levyFlight().*iwt(i).*vel...                             % prev vel
    ...
    % limit velocities here using masking 位置约束
