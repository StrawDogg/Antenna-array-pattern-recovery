 ...
 subplot('position',[0.1,0.1,.475,.825]);
  ...
  if D==1
     titstr1=sprintf(['%11.6g = %s( [ %9.6g ] )'],...
                gbestval,strrep(func2str(functname),'_','\_'),gbest(1));
  elseif D==2
     titstr1=sprintf(['%11.6g = %s( [ %9.6g, %9.6g ] )'],...
                gbestval,strrep(func2str(functname),'_','\_'),gbest(1),gbest(2));
  elseif D==3
     titstr1=sprintf(['%11.6g = %s( [ %9.6g, %9.6g, %9.6g ] )'],...
                gbestval,strrep(func2str(functname),'_','\_'),gbest(1),gbest(2),gbest(3));
  else
     titstr1=sprintf(['%11.6g = %s( [ %g inputs ] )'],...
                gbestval,strrep(func2str(functname),'_','\_'),D); %更改
  end
...
subplot('position',[.62,.1,.29,.4]);
  ...
  titstr={'PSO Model: '      ,PSOtype;...
          ...
          'Function : '      ,strrep(func2str(functname),'_','\_');...
          xtraname           ,xtraval;...
          rststat1           ,rststat2};
  ...
  if strcmp('pso_neteval',func2str(functname))
