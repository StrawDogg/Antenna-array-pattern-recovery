...
%% 获取SLL
SLL_Depth = findpeaks(F_dB_Nmlz_Phi0_Ipt); %找到主瓣和所有旁瓣的最高功率点
% [Peaks,~] = sort(Peaks);          %从小到大排序
% SLL = Peaks(end) - Peaks(end-1);  %用主瓣功率减去最大的旁瓣功率

% 筛选SLL
shaixuanSLL = find(SLL_Depth > -15);
SLL_Depth = SLL_Depth(shaixuanSLL);

SLL_index = zeros(1,length(SLL_Depth));
SLL_Angle = zeros(1,length(SLL_Depth));   %初始化零点角度向量

for i = 1:length(SLL_Depth)
    index = find(F_dB_Nmlz_Phi0_Ipt==SLL_Depth(i));  %找到零点的角度
    SLL_index(i) = index(1);
    SLL_Angle(i) = Theta_Ipt(SLL_index(i));
end
SLL = [SLL_index;SLL_Depth;SLL_Angle];

%% 获取零点
Nulls_Depth = -findpeaks(-F_dB_Nmlz_Phi0_Ipt); %找到所有最低功率点，即零点

% 筛选零点
shaixuan = find(Nulls_Depth < -45);
